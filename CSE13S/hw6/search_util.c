...
int score_letter(char letter, char **vocabulary, size_t num_words) {
  ...
  for (size_t i = 0; i < num_words; i++) {
    for (int j = 0; j < 5; j++) {
      if (vocabulary[i] != NULL && letter == vocabulary[i][j]) {
        score++;
        break;
      }
    }
...
int score_word(char *word, int *letter_scores) {
  // TODO(you): implement this function
  int score = 0;
  int visited_letters[5] = {0};
  for (int i = 0; i < (int)strlen(word); i++) {
    char letter = word[i];
    bool found = false;
    for (int j = 0; j < 5; j++) {
      if (letter == visited_letters[j]) {
        found = true;
      }
    }
    if (!found) {
      score += letter_scores[letter - 97];
    }
    visited_letters[i] = letter;
  }
  return score;
}
