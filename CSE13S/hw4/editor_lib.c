...
ll_text *append_text(ll_text *list, char *text) {
  if (list == NULL) {
    ll_text *output = calloc(1, sizeof(ll_text));
...
ll_text *replace_text(ll_text *list, char *text, int position) {
  int curr_pos = 0;
  ll_text *curr_ll = list;
  while (curr_pos != position) {
    curr_ll = curr_ll->next;
    curr_pos++;
  }
  free(curr_ll->text);
  curr_ll->text = strdup(text);
  return list;
}
...
ll_text *duplicate_ll_text(ll_text *list) {
  ...
  for (int i = 0; i < ll_text_length(list); i++) {
    front = append_text(front, curr_ll->text);
...
ll_text_stack *push_duplicate(ll_text_stack *stack) {
  ll_text_stack *output = calloc(1, sizeof(ll_text_stack));
  output->next = stack;
  output->lines = duplicate_ll_text(stack->lines);
  return output;
}
...
ll_text_stack *pop_stack(ll_text_stack *stack) {
  ...
  int len = ll_text_length(stack->lines);

  for (int i = 0; i < len; i++) {
    stack->lines = delete_text(stack->lines, 0);
  }
